...
Deno.serve(async (req: Request) => {
  ...
  // ------------------------------------------------------------------------
  // التوقيع إلزامي. النسخة السابقة كانت تتخطى الفحص إذا السرّ مو مضبوط
  // (`if (cfg?.app_secret) { … } else { warn }`) — يعني تفشل *مفتوحة*: أي
  // واحد يدز أحداث واتساب مزوّرة، يزوّر تقارير تسليم، والأخطر إنه يسجّل
  // رسالة واردة وهمية باسم رقم زبون فيفتح نافذة الـ٢٤ ساعة ويصير الإرسال
  // الحر ممكناً لذلك الرقم. الآن تفشل *مغلقة*.
  // ------------------------------------------------------------------------
  if (!cfg?.app_secret) {
    console.error("whatsapp_app_secret مو مضبوط بالخزنة — انرفض الطلب. " +
                  "ضيفه من Meta App Dashboard ← Settings ← Basic ← App Secret.");
    return new Response("webhook not configured", { status: 503 });
  }

  if (!await validSignature(cfg.app_secret, raw, req.headers.get("x-hub-signature-256"))) {
