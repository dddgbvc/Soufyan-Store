...
## التشغيل بلا npm (ملف واحد — للآيباد والجوال)

الملف `standalone/soufyan-erp-auth.html` نسخة مستقلة تمامًا: كل الشيفرة والتنسيقات
مدمجة داخله. لا يحتاج npm ولا Node ولا خادمًا — يكفي فتحه في المتصفح.

**على iPad / iPhone:**

1. احفظ الملف في تطبيق **الملفات** (iCloud Drive أو «على الآيباد»).
2. افتح تطبيق الملفات واضغط على الملف — يُعرض ويعمل مباشرة.

**على الحاسوب:** انقر الملف نقرًا مزدوجًا ليُفتح في المتصفح.

لإعادة توليده بعد أي تعديل على الشيفرة:

```bash
npm run build:single
```

> سبب عمله من `file://`: البناء يُنتج حزمة IIFE كلاسيكية مضمّنة داخل الصفحة،
> لأن وحدات ES لا تُحمَّل من نظام الملفات بسبب قيود CORS.
> الخط `Tajawal` يُحمَّل من الإنترنت إن توفّر، ومع انقطاعه تتولّى الخطوط النظامية
> العربية العرض دون أي كسر في التخطيط.
