...
| **Tests — تشغيل فعلي** | ✅ PASS | **215 فحصًا · صفر إخفاق** في Chromium حقيقي |
...
failures    37 pass · 0 fail
security    40 pass · 0 fail
demo        19 pass · 0 fail
────────────────────────────
المجموع    215 pass · 0 fail
```
...
## انحدار وقع بعد التسليم الأول — وأُصلح

**العرض:** «البرنامج ما يشتغل» — شاشة فارغة عند الفتح.

**السبب:** `Router.boot()` صار في V7 ينتظر `setup_status()` قبل رسم أي شاشة:

```js
async boot(){
  document.body.dataset.surface = "auth";
  const linkErr = this._hashError();
  await this.loadSetupStatus();        // ← يحجب أول رسم
  ...
```

على شبكة صامتة (خادم لا يردّ، لا ينقطع) يبقى النداء معلّقًا حتى
`requestTimeout` = ١٥ ثانية، والصفحة فارغة طوال ذلك. مقيس: **لم تظهر أي شاشة
خلال عشر ثوانٍ**. في V6 كانت البوابة تظهر فورًا لأن الإقلاع لم يكن يسأل الخادم
أصلًا في هذا المسار.

**الخطأ في التفكير:** عاملتُ «هل اكتمل الإعداد؟» كأنها قرار أمني يجب أن يُحسم
قبل أي شيء. وهي ليست كذلك — هي قرار **توجيه** بين شاشتين، والوجهة الافتراضية
(البوابة) تحمل البابين معًا فلا ضرر في عرضها قبل أن يجيب الخادم. القرار الأمني
الحقيقي هو الجلسة، وهو محفوظ خلف شاشة انتظار صريحة بمؤشّر دوّار.

**الإصلاح:** ترتيب مقلوب — ارسم فورًا، ثم صحّح الوجهة إن جاء الردّ وكان
المستخدم ما يزال على البوابة. ومهلة قصيرة (٦ ثوانٍ) خاصة بهذا النداء بدل
`requestTimeout` الكامل، حتى لا يقفز المستخدم من شاشة إلى أخرى بعد خمس عشرة
ثانية بلا سبب مفهوم.

**النتيجة:** ١٠+ ثوانٍ (لم تظهر) ← **١٦٩ مللي ثانية**.

**اختبار الانحدار:** `tests/failures.mjs` — «الإقلاع على شبكة صامتة»، ثلاثة
فحوص تقيس زمن أول رسم مقابل خادم لا يردّ إطلاقًا.

**الدرس:** لم تكشفه أيٌّ من المجموعات الأربع لأنها كلها كانت تعترض نداءات
Supabase وتردّ **فورًا**. لا اختبار كان يقيس السلوك مقابل خادم بطيء أو صامت —
وهي أشيع حالة في متجر بإنترنت ضعيف. الاعتراض الفوري يخفي فئة كاملة من العيوب.


---

## عيب ثانٍ ظهر مع الأول: تحديث الصفحة كان يُخرج المستخدم

**العرض:** تسجّل الدخول، تضغط F5، تعود إلى شاشة الدخول.

**السبب** — موروث من V6 لا من V7:

```js
window.addEventListener("pagehide", () => {
  navigator.sendBeacon(… "/rpc/app_session_end" …);   // "إغلاق التبويب"
});
```

لكن `pagehide` **يقع عند تحديث الصفحة أيضًا**، لا عند الإغلاق وحده. فكل تحديث:

```
pagehide → app_session_end (closed_at = now)
الإقلاع  → app_session_whoami → {"ok":false,"reason":"closed"}
⇒ شاشة الدخول
```

**الدليل** — نُفِّذ على قاعدة الإنتاج، لا استنتاجًا:

```
app_session_start → app_session_end → app_session_whoami
⇒ {"ok": false, "reason": "closed"}
```

**الإصلاح:** أُزيلت النبضة. التنظيف مغطًّى أصلًا: `app_session_start` يُغلق أي
جلسة سابقة مفتوحة على الجهاز نفسه، والنبضة الدورية تُحدّث `last_seen_at`،
ومهاجرة `cron_close_stale_sessions` تكنس المهملة.

### لماذا لم يكشفه أي اختبار — والدرس المشترك مع عيب الإقلاع

المحاكاة كانت **بلا ذاكرة**: `app_session_end` يردّ `{ok:true}` ولا يغيّر شيئًا،
و`whoami` ينجح دائمًا. فلا خادم يتذكّر أن الجلسة أُغلقت.

هذا هو الدرس نفسه الذي كشفه عيب الإقلاع بصيغة أخرى:

| العيب | ما أخفاه |
|---|---|
| شاشة فارغة ١٥ ثانية | المحاكاة تردّ **فورًا** — لا اختبار قابل خادمًا بطيئًا |
| التحديث يُخرج المستخدم | المحاكاة **بلا حالة** — لا اختبار قابل خادمًا يتذكّر |

**الإصلاح البنيوي:** `tests/contracts.mjs` صار يحفظ الجلسات المغلقة، فلو عادت
النبضة سقط الفحص فورًا. وأُضيفت مجموعة `demo` تختبر الافتراضي المُسلَّم نفسه
بدل أن يبقى بلا تغطية.


---

## عيب ثالث: الصفحة لا تُفتح على الآيباد

**العرض:** «لا يمكن لسفاري فتح هذه الصفحة لعدم وجود الإنترنت» — والإنترنت يعمل.
المعاينة عبر Textastic على iPad.

**سببان، كلاهما من إضافات V7:**

**١ · `upgrade-insecure-requests` في CSP.** أضفتُها ضمن ترويسات التصليب. أثرها
أن كل طلب `http://` يُرفع إلى `https://`. ومحرّرات iOS تعاين عبر خادم محلي على
`http://localhost:PORT`، فتُرفع الطلبات إلى `https://localhost:PORT` ولا شيء
هناك. أُزيلت من `<meta>` — ومكانها الصحيح ترويسة HTTP على خادم إنتاج بـTLS،
وهي موجودة أصلًا في `deploy/security-headers.conf`.

**٢ · `'self'` لا تُطابق `file://`.** سفاري يعامل أصل الملف كأصل معتم، فتُحجب
`vendor/*.js` عند الفتح المباشر. أُضيف `file:` إلى المصادر.

**والأعمق:** الصفحة تطلب خمسة موارد خارجية (ثلاثة في `vendor/` ووصلة خطوط)،
ومحرّرات iOS تعاين الملف وحده. فأُضيفت `tools/build-standalone.mjs` التي تُنتج
**ملفًا واحدًا قائمًا بذاته** — بلا `vendor/` وبلا خطوط بعيدة. مُتحقَّق منه في
مجلد فارغ تمامًا: صفر طلب غير `file://`، صفر طلب فاشل، صفر خطأ.

### وعيب رابع كشفته لقطة الشاشة

شارة «وضع تجريبي» **كانت مخفيّة على الآيباد**:

```css
@media (max-width:900px){ .mode-flag{ display:none } }   /* والآيباد ٨٣٤ */
```

أي أن الجهاز الذي تُجرَّب عليه النسخة غالبًا هو الوحيد الذي لا يرى التنبيه —
فتبدو نسخة لا تُنشئ حسابًا ولا تحفظ شيئًا كأنها تشغيل حقيقي. أُضيف شريط دائم
أعلى الصفحة لا يُخفى في أي عرض.

**ولماذا لم يكشفه الاختبار:** كان يقرأ `textContent` فقط، والنصّ موجود في DOM
وإن كان `display:none`. صار يقيس **الرؤية الفعلية** (`getBoundingClientRect` +
`getComputedStyle`) على أربعة عروض: ٣٩٠ · ٨٣٤ · ١٠٢٤ · ١٤٤٠.

### الخيط الجامع بين العيوب الأربعة

| العيب | ما أخفاه |
|---|---|
| شاشة فارغة ١٥ ثانية | المحاكاة تردّ فورًا — لا خادم بطيء |
| التحديث يُخرج المستخدم | المحاكاة بلا ذاكرة — لا خادم يتذكّر |
| لا تُفتح على الآيباد | الاختبار على Chromium/Linux وحده — لا سفاري ولا `file://` معزول |
| الشارة مخفيّة | الفحص يقرأ النصّ لا الرؤية — لا قياس عبر العروض |

كلها من نوع واحد: **بيئة الاختبار كانت أنظف من الواقع.** الإصلاح في كل مرة كان
تقريب البيئة من الواقع، لا ترقيع العَرَض.
